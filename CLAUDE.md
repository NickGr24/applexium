...
src/
  ...
  motion/                LenisProvider, PageTransition, ease.ts, useReducedMotion,
                          useSpotlightPointer
  scenes/                SceneCanvas + graphicsTier + the four R3F scenes
...
### 3D scenes — `SceneCanvas` + `graphicsTier`, always with a poster

`src/scenes/graphicsTier.ts` picks `'static' | 'lite' | 'high'` once per mount, based on `prefers-reduced-motion`, WebGL2 support, pointer coarseness, core count, and `navigator.deviceMemory` where available. `SceneCanvas` (`src/scenes/SceneCanvas.tsx`) is the only place that reads it directly: it always renders `poster` (a cheap CSS/image placeholder — see `.scene-poster--*` in `components.css`) as a permanently-mounted base layer, and only lazily mounts the real R3F `<Canvas>` (`SceneCanvasInner`, dynamically imported) once the `'static'` tier is ruled out **and** an `IntersectionObserver` confirms the section is near the viewport. **Every scene call site must supply a `poster`** — it's not optional chrome, it's the entire visual for SSR, reduced-motion visitors, low-tier devices, and the gap between "Canvas exists in the DOM" and "first WebGL frame actually painted."

The four scenes (`AuroraBackground`/`AuroraCanvas`, `BeamsScene`, `ConvergenceScene`, `GalaxyScene`) are each dynamically imported (`React.lazy`) from their page, never statically imported — that's what keeps `three`/`@react-three/*` (~880KB) out of every page's own JS chunk. If you add a fifth scene, keep that pattern, and add it to `fix-preload.mjs`'s `heavy` list check (or rather, its guard already matches on chunk-name prefixes like `*Scene-`; make sure the new file follows the same `*Scene.tsx` naming so the guard actually catches a regression).
