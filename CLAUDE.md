...
- **TEMPORARY (2026-09-04): the working copy lives in `~/Desktop`, which iCloud Drive syncs and evicts.** Evicted ("dataless") files hang `vite build` in `fs.copyFileSync` and make `node_modules` reads return empty modules. `node_modules` is therefore a symlink to `~/.cache/applexium-deps/node_modules` (installed from the same lockfile); run `npm ci` **there**, not here, and check `find . -flags +dataless | wc -l` if a build stalls (`brctl download <path>` re-fetches). The real fix is moving the repo out of `~/Desktop`; delete this paragraph and the symlink when that happens. **Observed 2026-09-19:** the symlink was gone — `node_modules` was a real directory again with ~1.9k evicted files, `~/.cache/applexium-deps` was empty, and ~1000 loose objects under `.git` plus ~100 files under `src/`/`public/` were evicted too, so `git status` and `grep -r` hung as well. What worked: `find . -path ./node_modules -prune -o -flags +dataless -print0 | xargs -0 -n 1 brctl download` (one path per call; takes minutes, poll until the count is 0), and running `npm ci && npm test && npm run build` from an rsync mirror of the sources at `~/.cache/applexium-build` instead of here.
...
`vite-react-ssg build` is only the first of five steps chained in the `build` script:

```
vite-react-ssg build  →  scripts/fix-preload.mjs  →  scripts/inline-css.mjs  →  scripts/gen-sitemap.mjs  →  scripts/verify-dist.mjs
```
...
3. **`scripts/inline-css.mjs`** inlines each page's stylesheets into one `<style>` and drops the `<link rel="stylesheet">` tags (the `.css` files still ship: client-side navigation loads other routes' CSS through Vite's preload helper, and a one-line script adds `disabled` marker links so hydration doesn't re-download what is already inline). **This, together with `Seo.tsx` deliberately NOT preloading a font, is what holds mobile Lighthouse at 99–100 instead of 88–92** — read the header of that script before touching either half. The mechanism is counter-intuitive: Lighthouse/PSI record when the first paint was *observed* and charge every script evaluated before it to FCP and LCP; if the document can't render the instant its HTML arrives (a stylesheet one round trip away, or a font preload's short render hold), Lighthouse's Chrome skips that first frame and paints a full second later, after all the JS. Locally this never reproduces (assets arrive in ~2ms) — to measure, serve `dist/` with production latency (230ms HTML, 45ms assets, HTTP/2) and run Lighthouse 3–4 times; a single run proves nothing, the same build scored 89 and 100 back to back before the fix.
4. **`scripts/gen-sitemap.mjs`** writes `dist/sitemap.xml` from `src/site/pages.json` and copies `dist/en.html` → `dist/en/index.html` (GitHub Pages needs a real `index.html` for the `/en/` directory URL).
5. **`scripts/verify-dist.mjs`** is the guard: it asserts a list of invariants across all rendered pages (no `<link rel="stylesheet">` and no font/style preload in any page, the CSS marker script matching the inlined files, hreflang completeness, JSON-LD present, canonical present, RO/EN stylesheet-link parity, no lazy scene chunk ever modulepreloaded, legal pages preload exactly their own content chunk, sitemap excludes `/projects`, `dist/en/index.html` byte-identical to `dist/en.html`, etc.) and **exits non-zero, printing every violation**, if any fail.
...
- **An infinite CSS animation is not free here, even off screen — gate it on visibility.** ScrollTrigger keeps a `requestAnimationFrame` alive for the whole visit (its internal `_rafBugFix`, on top of the GSAP ticker that drives Lenis), so the browser produces a main frame every vsync and re-ticks every *running* CSS animation in each one — including "compositor-only" transforms, including elements inside a `content-visibility`-skipped section. The 2026-09-19 weak-device pass measured the home page's client marquee that way (phone profile, CPU ×6, idle, strip 1500px off screen): 120 style recalcs/s and 13.6% of the main thread, against 0 and 6.2% once `ClientLogos` (`src/pages/home/ClientLogos.tsx`) pauses it via an IntersectionObserver. Any new looping animation needs the same gate; `tests/clients.test.tsx` pins this one. The remaining ~6% idle floor is the GSAP/ScrollTrigger/Lenis loops themselves.
- **No `mask-image` over animating content.** The marquee's edge fades are two static gradient overlays in `--bg`, not a mask: a mask over a perpetually-moving child is re-composited through an offscreen surface every frame. It only works because the ground behind that section is flat.
...
- **Adding a client logo / portfolio project** → drop the full-size artwork in `public/` (used by `/projects`' `ProjectCard`), then make a marquee variant at 2× the chip's displayed size under `public/logos/marquee/` (84px tall for wordmarks, capped at 480px wide; 128px for square `tall` marks) and list it in `src/site/clients.ts` with its real `srcW`/`srcH`. `tests/clients.test.tsx` fails if a marquee file is over 20KB, taller than 128px, or its pixel size disagrees with `srcW`/`srcH` — the marquee used to decode the full-size originals (11.6MB of bitmap for one row of chips). A `/projects` card also needs `projects.cases.<key>.{name,text,tag}` in both dictionaries. Full case studies are a separate, bigger thing (`site/cases.ts`, a route, `tests/cases.test.ts` pins exactly three).
