/*!
 * LiquidEther — ReactBits fluid simulation
 * Requires Three.js (loaded via CDN)
 * Usage: initLiquidEther(selector, { colors, mouseForce, ... })
 */
function initLiquidEther(selector, userOpts) {
  var _container = document.querySelector(selector);
  if (!_container) return;

  var _opts = Object.assign({
    mouseForce: 38, cursorSize: 25, isViscous: false, viscous: 30,
    iterationsViscous: 32, iterationsPoisson: 58, dt: 0.014,
    BFECC: true, resolution: 0.4, isBounce: false,
    colors: ['#5227FF', '#FF9FFC', '#B19EEF'],
    autoDemo: true, autoSpeed: 0.5, autoIntensity: 2.2,
    takeoverDuration: 0.25, autoResumeDelay: 1000, autoRampDuration: 0.6
  }, userOpts || {});

  var mouseForce = _opts.mouseForce, cursorSize = _opts.cursorSize,
      isViscous = _opts.isViscous, viscous = _opts.viscous,
      iterationsViscous = _opts.iterationsViscous,
      iterationsPoisson = _opts.iterationsPoisson,
      dt = _opts.dt, BFECC = _opts.BFECC,
      resolution = _opts.resolution, isBounce = _opts.isBounce,
      colors = _opts.colors,
      autoDemo = _opts.autoDemo, autoSpeed = _opts.autoSpeed,
      autoIntensity = _opts.autoIntensity,
      takeoverDuration = _opts.takeoverDuration,
      autoResumeDelay = _opts.autoResumeDelay,
      autoRampDuration = _opts.autoRampDuration;

  var _raf = null, _resizeObs = null, _interObs = null;
  var _isVisible = true, _webgl = null, _resizeRaf = null;
    ...
    class MouseClass {
      ...
      init(_container) {
        this.container = _container;
        this.docTarget = _container.ownerDocument || null;
        const defaultView =
...
`;
    ...
    const applyOptionsFromProps = () => {
      if (!_webgl) return;
      const sim = _webgl.output?.simulation;
      if (!sim) return;
  ...
  return function destroy() {
    ...
    if (_interObs) try { _interObs.disconnect(); } catch(e) {}
    if (_webgl) _webgl.dispose();
    _webgl = null;
  };
}